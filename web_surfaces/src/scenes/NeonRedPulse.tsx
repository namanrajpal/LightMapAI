...
// CRAZY color-cycling neon — offset hue from blue scene

export function NeonRedPulse() {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // Offset hue — 120° ahead of blue scene
      const baseHue = (t * 40 + 120) % 360

      ctx.fillStyle = hsl(baseHue, 60, 5)
      ctx.fillRect(0, 0, w, h)

      // Center pulse
      const centerPulse = 0.3 + Math.sin(t * 1.8) * 0.2
      const grad = ctx.createRadialGradient(w / 2, h / 2, 0, w / 2, h / 2, Math.max(w, h) * 0.7)
      grad.addColorStop(0, hsl(baseHue + 20, 80, 25, centerPulse))
      grad.addColorStop(1, 'rgba(0,0,0,0)')
      ctx.fillStyle = grad
      ctx.fillRect(0, 0, w, h)

      // Edge glow
      const edgeW = 30 + Math.sin(t * 2.5) * 15
      const edgeDefs = [
      ...
      for (const edge of edgeDefs) {
        const pulse = 0.5 + Math.sin(t * 2.8 + edge.phase * 1.3) * 0.4
      ...
      ctx.strokeStyle = hsl(borderHue + 40, 100, 50, 0.35 + Math.sin(t * 2.2) * 0.25)
      ctx.lineWidth = 3
      ctx.strokeRect(inner, inner, w - inner * 2, h - inner * 2)

      // Traveling light — opposite direction
      const perim = 2 * (w + h)
      const pos = perim - ((t * 250) % perim)
      ...
      // Random flash
      if (Math.random() > 0.88) {
        ctx.fillStyle = hsl(baseHue + Math.random() * 180, 100, 50, 0.06 + Math.random() * 0.12)
        ctx.fillRect(0, 0, w, h)
      }
    }
