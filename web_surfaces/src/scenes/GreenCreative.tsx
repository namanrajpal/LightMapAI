...
// CRAZY color-cycling neon — offset hue from the other two

export function GreenCreative() {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // Offset hue — 240° ahead of blue scene
      const baseHue = (t * 40 + 240) % 360

      ctx.fillStyle = hsl(baseHue, 60, 5)
      ctx.fillRect(0, 0, w, h)

      // Center pulse
      const centerPulse = 0.3 + Math.sin(t * 2.2) * 0.2
      const grad = ctx.createRadialGradient(w / 2, h / 2, 0, w / 2, h / 2, Math.max(w, h) * 0.7)
      grad.addColorStop(0, hsl(baseHue + 15, 80, 25, centerPulse))
      grad.addColorStop(1, 'rgba(0,0,0,0)')
      ctx.fillStyle = grad
      ctx.fillRect(0, 0, w, h)

      // Edge glow
      const edgeW = 30 + Math.sin(t * 3.2) * 15
      const edgeDefs = [
      ...
      for (const edge of edgeDefs) {
        const pulse = 0.5 + Math.sin(t * 3 + edge.phase * 1.1) * 0.4
      ...
      ctx.strokeStyle = hsl(borderHue + 40, 100, 50, 0.35 + Math.sin(t * 2.8) * 0.25)
      ctx.lineWidth = 3
      ctx.strokeRect(inner, inner, w - inner * 2, h - inner * 2)

      // TWO traveling lights — opposite directions
      const perim = 2 * (w + h)
      for (let d = 0; d < 2; d++) {
        const pos = d === 0 ? ((t * 280) % perim) : (perim - ((t * 220) % perim))
        ...
        spotGrad.addColorStop(0, hsl(baseHue + 90 + d * 120, 100, 70, 0.9))
        spotGrad.addColorStop(0.4, hsl(baseHue + 90 + d * 120, 100, 50, 0.4))
      ...
      // Random flash
      if (Math.random() > 0.85) {
        ctx.fillStyle = hsl(baseHue + Math.random() * 180, 100, 50, 0.06 + Math.random() * 0.15)
        ctx.fillRect(0, 0, w, h)
      }
    }
